...
@section('title', $currentView == 'pending' ? 'Pending Comments' : 'Published Comments')
    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">{{ $currentView == 'pending' ? 'Pending Comments' : 'Published Comments' }}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard.home') }}">Dashboard</a></li>
                            <li class="breadcrumb-item active">
                                {{ $currentView == 'pending' ? 'Pending Comments' : 'Published Comments' }}
                            </li>
                        </ol>
                    </div>
                </div>
                        ...
                        <div class="card">
                            <div class="card-header p-0 pt-1 border-bottom-0">
                                <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link {{ $currentView == 'published' ? 'active' : '' }}"
                                            href="{{ route('dashboard.comments.index') }}">Published Comments</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link {{ $currentView == 'pending' ? 'active' : '' }}"
                                            href="{{ route('dashboard.comments.index', ['view' => 'pending']) }}">Pending
                                            Comments</a>
                                    </li>
                                </ul>
                            </div>
                                            ...
                                            @forelse ($comments as $comment)
                                                <tr>
                                                    ...
                                                    <td class="text-center"><a
                                                            href="{{ route("dashboard.comments.status", $comment->id) }}"><span
                                                                class="badge bg-{{ $comment->status ? "success" : "warning" }}">{{ $comment->status ? "Published" : "Pending" }}</span></a>
                                                    </td>
                                                    <td class="text-center">
                                                        <div class="d-flex justify-content-center">
                                                            <a data-href="{{ route("dashboard.comments.show", $comment->id) }}"
                                                                class="btn btn-primary commentdetails">Details</a>
                                                            <form
