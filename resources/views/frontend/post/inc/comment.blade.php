                            ...
                            <div class="content">
                                <div class="meta">
                                    <ul class="list-inline">
                                        @if ($comment->user)
                                            <li><a
                                                    href="{{ route("frontend.user", $comment->user->username) }}">{{ $comment->user->name }}</a>
                                            </li>
                                        @else
                                            <li>{{ $comment->name }}</li>
                                        @endif
                                        @if ($comment->user && $comment->user == $post->user)<span
                                        class="badge bg-success ml-2 p-1 text-white">Post Creator</span>@endif
                                        <li class="slash"></li>
                                        <li>{{ $comment->created_at->diffForHumans() }}</li>
                                    </ul>
                                </div>
                                <p>{{ $comment->message }}</p>
                                <a href="#" data-comment-id="{{ $comment->id }}" class="btn-reply"><i class="las la-reply"></i>
                                    Reply</a>
                            </div>
                        </div>
                                            ...
                                            <div class="content">
                                                <div class="meta">
                                                    <ul class="list-inline">
                                                        @if ($reply->user)
                                                            <li><a
                                                                    href="{{ route("frontend.user", $reply->user->username) }}">{{ $reply->user->name }}</a>
                                                            </li>
                                                        @else
                                                            <li>{{ $reply->name }}</li>
                                                        @endif
                                                        @if ($reply->user && $reply->user == $post->user)<span
                                                        class="badge bg-success ml-2 p-1 text-white">Post Creator</span>@endif
                                                        <li class="slash"></li>
